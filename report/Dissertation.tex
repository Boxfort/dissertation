...
\input{reqspec}
\subsection{Testing}
TODO: Paragraph about testing, unit testing and why
...
TODO : Re-test results, I think I need to make changes to classifier configurations, thats why second set is getting real bad results. 
...
\subsection{Single vs Two-stage Classifiers Performance}
\begin{itemize}
\item{knn-knn second stage for unsw results in same normals but lower attack classification. Attacks require normals to better differentiate between them.}
\item{svm-svm normal has less true positives meaning that attacks will all have higher false positive rates. Also means that individual attacks are important in defining boundaries between normal and attack, not just as a whole.}
\item{Combining classifiers with less performant classifiers will always reduce performance}
\item{in NSL KDD, adding a more performant will generally increase performance}
\item{in NSL KDD combining with self will produce better results, except in the case of SVM}
\end{itemize}
