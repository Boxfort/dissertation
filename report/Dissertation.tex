...
One hot encoding is necessary as many classification algorithms rely on either some kind of internal averaging, or some form of activation function which require numerical input in order to extract meaning from. Simply converting each of these categories from a nominal to a numeric representation is not sufficient as the categories lose meaning when represented by a series of numbers, i.e. when calculating distance from one category to another any number selected will be arbitrary, producing results of no real meaning therefore reducing accuracy in classification. One hot encoding prevents this by having all categories the same distance from one another, and also within use in activation functions, something can only be in a category or not in a category, ensuring that when feature scaling is introduced a category cannot be for example 0.5. 
